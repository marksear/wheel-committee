...
function formatContract(contract, currentPrice, dte) {
  const mid = (contract.bid != null && contract.ask != null)
    ...
    : null // Don't fall back to lastPrice — it's stale and misleads premium calculations
    ...
    if (options.chains && options.chains.length > 0) {
      ...
      for (const chain of options.chains) {
        ...
        if (relevantPuts.length > 0) {
          section += `OTM PUTS (for cash-secured puts) — USE the **Mid** column as the premium:\n`
        ...
        if (relevantCalls.length > 0) {
          section += `OTM CALLS (for covered calls) — USE the **Mid** column as the premium:\n`
          section += `| Strike | Bid | Ask | **Mid** | IV% | OI | Vol |\n`
          section += `|--------|-----|-----|---------|-----|-----|-----|\n`
